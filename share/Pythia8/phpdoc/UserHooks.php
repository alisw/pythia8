...
<br/>(ix) Ones that allow an enhanced rate of rare shower splittings. 
...
<h3>(ix) Enhanced rate of rare shower splittings</h3> 
...
<code>UserHooks</code>, but cannot be used at the same time. 
...
same <code>name</code> arguments as above. An optimal choice depends on 
...
<p/>Trial emission enhancements also necessitate corrective weights. 
These are handled internally, so that users only have to ensure that 
<code>Info::mergingWeight()</code> or 
<code>Info::mergingWeightNLO()</code> are correctly taken into account. 
 
<p/>A simple example of enhanced regular emissions is provided in 
<code>main63.cc</code>, whereas <code>main64.cc</code> illustrates 
the usefulness of enhanced trial emissions for expert users. 
 
<h3>(x) Modified hadronization</h3> 
...
<br/><code>argument</code><strong> flavPtr </strong>  :  is a pointer to a <code>StringFlav</code> 
...
new parameters in a <code>Settings</code> object and reinitializing 
with <code>flavPtr->init(settings,randomPtr)</code>, where 
<code>settings</code> is the settings object and <code>randomPtr</code> 
is a pointer to the desired random number generator. Note that the 
<code>UserHooks</code> base class already holds pointers to the 
<code>Settings</code> and <code>Random</code> objects. 
   
<br/><code>argument</code><strong> zPtr </strong>  :  is a pointer to a <code>StringZ</code> object, 
which selects the hadron momentum fraction <i>z</i> using the Lund 
fragmentation function. Parameters are changed by 
<code>zPtr->init(settings,particleData,randomPtr)</code> 
in the same way as above. 
   
<br/><code>argument</code><strong> pTPtr </strong>  :  is a pointer to a <code>StringPT</code> object 
which selects the hadron <i>pT</i> from a Gaussian distribution. 
Parameters are changed by 
<code>pTPtr->init(settings,particleData,randomPtr)</code> 
in the same way as above. 
   
<br/><code>argument</code><strong> idEnd </strong>  :  gives the code of the parton (quark, diquark or 
gluon) in the string end that we are currently hadronizing from. 
   
<br/><code>argument</code><strong> m2Had </strong>  :  gives the invariant mass squared of all hadrons 
produced from the current end of the string up to this point. Used to 
keep track of where we are on the string. 
   
<br/><code>argument</code><strong> iParton </strong>  :  contains the indices in the standard event 
